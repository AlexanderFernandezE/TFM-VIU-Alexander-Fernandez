...
# 1. Leer la base
datos <- read_excel("Consolidado_Noticias_2020_2024.xlsx")

# 2. Extraer la fecha (solo la parte antes del guion bajo)
datos <- datos %>%
  mutate(
    fecha_texto = str_trim(str_extract(date, "^[0-9]{2}/[0-9]{2}/[0-9]{4}")),
    fecha = dmy(fecha_texto),
    anio = year(fecha)
  )

# 3. Contar noticias por año
frecuencia_anual <- datos %>%
  count(anio, name = "total_noticias")

# 4. Graficar líneas
ggplot(frecuencia_anual, aes(x = anio, y = total_noticias, group = 1)) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  labs(
    title = "Número anual de noticias sobre robos",
    x = "Año",
...
#NUBE DE PALABRAS DEL CONTENIDO TEXTUAL
#se filtró previamente la data a solo noticias asociadas a robo
#generándose otra base de datos (con otro nombre)
#Filtrado de datos:
...
# 2. Lista de palabras
palabras <- c("Asalto","Asaltos","Asaltante","Asaltantes","Asaltar","Asaltaron","Asalta",
              "Delincuencia","Delincuente","Delincuentes","Delinquir",
              "Robos","Robo","Robaba","Robaban","Robaron","Rateros","Ratero","Roba","Robar",
...
# 3. Filtrar filas donde AL MENOS UNA palabra tenga "SI"
datos_filtrados <- datos %>%
  filter(if_any(all_of(palabras), ~ . == "SI"))

# 4. (Opcional) Mantener solo esas columnas + info base
...
#Ahora sí se procede a realizar la Nube de palabras del contenido textual (es decir considera title y summary)
...
# 1. Leer la base
datos <- read_excel("datos_filtrados_robo.xlsx")

# 2. Unir título y resumen (ajusta nombres si varían)
texto <- paste(datos$title, datos$summary, sep = " ")
...
corpus <- corpus %>%
  ...
  tm_map(removePunctuation) %>%                   # quitar puntuación
  tm_map(removeNumbers) %>%                       # quitar números
  tm_map(removeWords, stopwords("spanish")) %>%   # quitar palabras vacías
  tm_map(stripWhitespace)                         # quitar espacios extra

# 5. Matriz de términos
tdm <- TermDocumentMatrix(corpus)

# 6. Convertir a matriz y sumar frecuencias
matriz <- as.matrix(tdm)
frecuencia <- sort(rowSums(matriz), decreasing = TRUE)

# 7. Nube de palabras
wordcloud(
  words = names(frecuencia),
  freq = frecuencia,
  min.freq = 5,                 # puedes ajustar
  max.words = 100,              # opcional
  colors = brewer.pal(8, "Dark2")
)
